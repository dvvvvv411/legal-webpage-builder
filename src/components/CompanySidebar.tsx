...
const CompanySidebar = ({ lawFirm, totalReviews, averageRating }: CompanySidebarProps) => {
  ...
  return (
    <>
      {/* Main sidebar content - hidden on mobile/tablet */}
      <aside className="sticky top-16 z-20 hidden lg:block">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 lg:hidden bg-white border-t border-neutral-200 shadow-lg">
        <div className="px-4 py-3 flex flex-col gap-2.5">
          ...
          <div className="flex gap-2.5">
            <Button variant="orange-outline" className="flex-1 py-4">
              <Phone className="w-4 h-4" />
              <span className="sr-only">Anrufen</span>
            </Button>
            <Button 
              variant="orange-outline-no-hover" 
              className="flex-1 text-lg py-4 rounded-lg"
              onClick={() => navigate(reviewLink)}
            >
              Bewertung abgeben
            </Button>
